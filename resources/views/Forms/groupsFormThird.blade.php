...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased h-screen w-screen flex justify-center items-center" style="background-image: url('/bg/gplay.png')">
        <div class="h-auto bg-gray-100 border-2 rounded-lg w-auto flex justify-center">
            <div><form action="{{route('processForm')}}" method="post">
                <input type="hidden" name="groupName" value="{{$groupName}}">
                <input type="hidden" name="classId" value="{{$classId}}">
                <input type="hidden" name="students" value="{{$students}}">
                <div class="flex justify-center"><div><button class="text-white bg-gray-800 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-auto px-10 py-2 my-4" onclick="createNew();" type="button">Neues Datum</button></div></div>
                <div id="dateContainer" class="grid gird-cols-3 mx-4 gap-2">
                ...
                <div class="flex justify-center"><div><input class="text-white bg-gray-800 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-auto p-2 my-4" type="submit"></div></div>
            </form></div>
        </div>
    </body>
    <script>
